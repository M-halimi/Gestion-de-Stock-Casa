...
export default function ProductsEdit({ product, categories, units }) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="heading-md text-ink">{t('pages.products.edit_title')}</h2>}>
            <Head title={t('pages.products.edit_title')} />

            <PageHeader
                title={t('pages.products.edit_title')}
                actions={
                    ...
                    <Card>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-hairline bg-canvas-soft">
                                {preview || data.image ? (
                                    <img src={preview ?? `/storage/${data.image}`} alt="" className="h-full w-full object-cover" />
                                ) : (
